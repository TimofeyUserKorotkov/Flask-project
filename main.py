...
def add_recipes():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            src = '../' + '/'.join(
                [app.config['UPLOADED_IMAGES_FOLDER'], filename])
            
            print(src)
            recipes.image = src
            print(recipes.image)
            file.save(os.path.join(
                app.config['UPLOADED_IMAGES_FOLDER'], 
                filename
                ))
...
def snack():
    ...
    if current_user.is_authenticated:
        recipes = db_sess.query(Recipes).filter(
            ((Recipes.user == current_user) | 
             (Recipes.is_private != True)), 
            Recipes.category == 'Перекус'
            )
        
    else:
        recipes = db_sess.query(Recipes).filter(
            Recipes.is_private != True, Recipes.category == 'Перекус'
            )
